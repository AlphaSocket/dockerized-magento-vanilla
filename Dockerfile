...
ENV \
	...
	BUILD_BRANCH="latest-cached" \
	BUILD_VERSION="latest-cached" \
	BUILD_ENV="cached" \
...
ADD imports/bin/setup /usr/local/bin/setup/1518734942
ADD imports/bin/config /usr/local/bin/config/1518734942
...
RUN chmod +x -R /usr/local/bin && \
    sync && \
    /usr/local/bin/setup/1518734942 1>/dev/stdout 2>/dev/stderr
